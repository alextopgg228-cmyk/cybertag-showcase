...
const replacements = {
  "В рамках практики разработана информационная система интернет-магазина оборудования для лазертага CYBERTAG. Система обеспечивает хранение сведений о товарах, категориях, клиентах, заказах и транзакциях, а также предоставляет адаптивный веб-интерфейс. Для администратора реализована панель управления каталогом с аутентификацией на основе JWT-токенов.":
    "В рамках практики разработана информационная система CYBERTAG с адаптивным многостраничным веб-интерфейсом, серверным приложением Node.js/Express и базой данных Microsoft SQL Server 2022 Express. Реализованы регистрация, SQL-заказы, серверные сессии и закрытая административная панель для управления заказами и акциями.",
  "реализованы таблицы, ограничения целостности, представления, хранимые процедуры и пользовательские функции в MS SQL Server 2022;":
    "реализованы 9 таблиц, ограничения целостности, 5 представлений, 4 хранимые процедуры, 3 пользовательские функции и 15 параметризованных сценариев запросов в Microsoft SQL Server 2022 Express;",
  ...
  "В системе выделяются три роли. Гость (незарегистрированный посетитель) просматривает витрину и формирует корзину. Покупатель (customer) оформляет заказы и отслеживает их статус в личном кабинете. Администратор (admin) управляет каталогом товаров, обрабатывает заказы, создаёт пользовательские учётные записи и просматривает статистику базы данных.":
    "В системе выделяются гость, зарегистрированный пользователь (customer) и администратор (admin). Гость просматривает страницы и формирует корзину. Зарегистрированный пользователь оформляет SQL-заказ после авторизации. Администратор manager просматривает все заказы, изменяет их статус и добавляет акции.",
  "Центральными таблицами являются Products и Orders, к которым обращаются наиболее часто. Таблица OrderItems содержит внешние ключи на Orders и Products, обеспечивая хранение состава каждой покупки без дублирования данных о товаре в каждой строке заказа. Для демонстрации работы системы подготовлен учебный набор данных: 6 категорий, 20 товаров, 10 клиентов, 15 заказов, 30 позиций заказа, 15 платёжных транзакций и 3 учётные записи пользователей.":
    "Предметная модель содержит таблицы каталога, заказов и Promotions, а контур авторизации использует Customers, Users и Sessions. При инициализации создаются 3 категории, 5 комплектов, 3 акции и административная учётная запись manager. Новые пользователи, заказы и сессии появляются после действий в интерфейсе.",
  ...
  "Административный поток изменяет состояние данных. После успешного входа администратор получает доступ к панели управления каталогом. При добавлении нового товара браузер отправляет POST-запрос с данными формы. Сервер проверяет JWT-токен и роль, затем выполняет INSERT в таблицу Products. При изменении или удалении — UPDATE или DELETE соответственно. Основные потоки данных системы представлены в таблице ниже:":
    "Административный поток начинается после проверки серверной сессии и роли admin. Панель запрашивает заказы через GET, изменяет статус через PATCH, добавляет акции через POST и снимает их с публикации через DELETE. Гость и customer не получают содержимое /admin/. Основные потоки данных представлены в таблице ниже:",
  "спроектировать реляционную базу данных CybertagShop с 7 таблицами;":
    "спроектировать реляционную базу данных CybertagShop с 9 таблицами, включая Users, Sessions и Promotions;",
  "разработать 10 параметризованных T-SQL запросов (04_Queries.sql);":
    "разработать 15 параметризованных T-SQL сценариев запросов (sql/04_Queries.sql);",
  ...
  "реализовать панель администратора с управлением каталогом и заказами;":
    "реализовать закрытую панель администратора для просмотра заказов, изменения статуса, добавления и удаления акций;",
  ...
  "Информационная система выполняет функции просмотра каталога товаров, фильтрации по категории и ценовому диапазону, полнотекстового поиска по наименованию, управления корзиной, оформления заказа, аутентификации пользователей, администрирования каталога, обработки заказов и отображения статистики базы данных.":
    "Информационная система выполняет функции просмотра оборудования, управления корзиной, регистрации, создания SQL-заказа, проверки сессии, просмотра всех заказов администратором, изменения статуса, публикации и удаления акций.",
  "Логическая модель базы данных строится вокруг товаров, клиентов, заказов, платёжных транзакций и пользователей системы. Справочные данные о группах товаров вынесены в отдельную таблицу Categories с поддержкой иерархии через поле ParentId. Это позволяет не повторять название категории в каждой строке таблицы Products.":
    "Логическая модель базы CybertagShop содержит таблицы каталога, заказов, Promotions и рабочий контур аутентификации. Customers хранит имя и email, Users — логин, bcrypt-хэш и роль, Sessions — SHA-256-хэш токена и срок действия. Promotions хранит описание, срок и изображение акции.",
  "Таблица OrderItems является связующей: она объединяет заказ и товар, фиксируя количество и цену на момент покупки. Поле UnitPrice в OrderItems хранит историческую цену, независимую от текущей цены товара. Таблица Users хранит учётные записи, связанные с записями покупателей через необязательный внешний ключ CustomerId.":
    "OrderItems связывает заказ и товар, фиксируя количество и историческую цену. Users связан с Customers, Sessions и Promotions. Sessions удаляется каскадно вместе с пользователем, а CreatedByUserId в Promotions фиксирует автора акции.",
  ...
  "Параметризованные запросы разработаны в файле 04_Queries.sql. Все запросы принимают входные параметры через конструкцию @ParameterName, что исключает SQL-инъекции при вызове из серверного API. Запросы охватывают все сценарии работы системы: просмотр витрины, управление заказами, аутентификацию и администрирование. Перечень запросов с описанием и ключевыми параметрами представлен в таблице ниже:":
    "Параметризованные запросы находятся в sql/04_Queries.sql. Пятнадцать сценариев охватывают регистрацию, сессии, каталог, заказы, изменение статуса, чтение, добавление и снятие акций с публикации. Значения передаются через параметры mssql.",
  ...
  "Реализован серверный REST API на Node.js/Express с подключением к SQL Server через пакет mssql. Аутентификация построена на JWT-токенах с разграничением ролей администратора и покупателя. Разработана адаптивная клиентская часть на HTML5, CSS3 и Vanilla JavaScript, включающая витрину товаров, систему фильтрации, корзину, форму оформления заказа и административную панель.":
    "Реализован REST API Node.js/Express с подключением через mssql/msnodesqlv8. Регистрация сохраняет Customers и Users, заказы — Orders и OrderItems, акции — Promotions, а сессии — Sessions. Панель manager защищена проверкой роли admin.",
  ...
  "В.3 Панель администратора": "В.3 Панель администратора",
  "Панель включает четыре блока: «Добавить товар» — форма с полями «Название», «Категория», «Цена», «Остаток», «Описание», «Изображение»; «Управление каталогом» — таблица всех товаров с кнопками «Изменить» и «Удалить»; «Заказы» — таблица заказов с возможностью изменить статус; «Статистика» — сводная таблица счётчиков по всем таблицам БД.":
    "Панель администратора содержит форму добавления акции с календарями даты начала и окончания, список действующих акций с кнопками удаления и таблицу всех заказов. Для заказа отображаются клиент, состав, сумма и статус. Сервер отдаёт /admin/ только роли admin; остальные пользователи перенаправляются.",
  "Первое звено — браузер пользователя: HTML/CSS/JavaScript интерфейс, HTTP-запросы с JWT в заголовках. Второе звено — серверное приложение Node.js/Express: маршрутизация, проверка токена, формирование SQL-запросов. Третье звено — СУБД MS SQL Server 2022: хранение, выборка, изменение и резервное копирование данных.":
    "Первое звено — браузер с HTML/CSS/JavaScript и HttpOnly cookie. Второе — Node.js/Express: валидация, bcrypt, проверка роли, API заказов и акций. Третье — Microsoft SQL Server 2022 Express: Customers, Users, Sessions, Orders, OrderItems и Promotions.",
...
setTable(0,
  ...
  [
    ...
    ["Акции (Promotions)", "Публикации администратора", "Название, срок, изображение"]
  ]);
...
setTable(1,
  ...
  [
    ...
    ["Пользователь", "POST /api/orders", "Проверка сессии + pr_CreateOrder", "Orders и OrderItems"],
    ["Администратор", "GET /api/admin/orders", "Проверка роли admin", "Все заказы"],
    ["Администратор", "PATCH /api/admin/orders/:id/status", "Валидация статуса", "Обновлённый заказ"],
    ["Администратор", "POST /api/admin/promotions", "Валидация + INSERT", "Новая акция"],
    ["Администратор", "DELETE /api/admin/promotions/:id", "Проверка роли + UPDATE", "IsActive = 0"],
...
setTable(2,
  ...
  [
    ["MS SQL Server 2022 Express", "Реляционная СУБД", "9 таблиц, ограничения, объекты БД"],
    ...
    ["Node test", "Интеграционное тестирование", "Регистрация, заказ, роли, статус, акция"]
  ]);
...
setTable(3,
  ...
  [
    ...
    ["Оформление", "Корзина / Контакты", "Orders + OrderItems в SQL"],
    ["Заказы администратора", "Панель", "Все клиенты, состав, сумма и статус"],
    ["Изменение статуса", "Панель", "new / processing / shipped / done / cancelled"],
    ["Добавление акции", "Панель", "Новая строка Promotions"],
    ["Удаление акции", "Панель", "Снятие с публикации через IsActive"],
...
setTable(4,
  ...
  [
    ...
    ["Promotions", "Акции", "PromotionId, Title, DateLabel, CreatedByUserId"]
  ]);
...
setTable(7,
  ...
  [
    ...
    ["FK_Promotions_Users", "Promotions(CreatedByUserId)", "Users(UserId)"]
  ]);
...
setTable(11,
  ...
  [
    ...
    ["11", "Получить все заказы", "роль admin"],
    ["12", "Изменить статус заказа", "@OrderId, @Status"],
    ["13", "Получить действующие акции", "IsActive"],
    ["14", "Добавить акцию", "@Title, @DateLabel, @UserId"],
    ["15", "Снять акцию с публикации", "@PromotionId"]
  ]);
...
setTable(13,
  ...
  [
    ...
    ["Нет прав администратора", "Недостаточно прав.", "Ошибка 403"],
    ["Заказ оформлен", "Заказ №N сохранён.", "Успех 201"],
    ["Акция добавлена", "Акция добавлена.", "Успех 201"],
    ["Акция удалена", "Акция снята с публикации.", "Успех 200"],
...
setTable(14,
  ...
  [
    ...
    ["Серверный", "Express, bcryptjs, cookie-parser", "Валидация, роли, API, сессии"],
    ["Данных", "SQL Server 2022, mssql/msnodesqlv8", "Users, Sessions, Orders, Promotions"]
  ]);
...
setTable(15,
  ...
  [
    ...
    ["Users", "1:N", "Promotions", "Нет", "Автор акции"]
  ]);
